-- 🚀 TAXSAGE SUPABASE DATABASE SCHEMA
...
-- Create RLS policies (users can only see their own data) - with IF NOT EXISTS equivalent
DO $$ 
BEGIN
    -- Drop existing policies if they exist, then recreate
    DROP POLICY IF EXISTS "Users can view their own profile" ON user_profiles;
    DROP POLICY IF EXISTS "Users can view their own income" ON income_entries;
    DROP POLICY IF EXISTS "Users can view their own expenses" ON expense_entries;
    DROP POLICY IF EXISTS "Users can view their own budgets" ON budget_allocations;
    ...
    DROP POLICY IF EXISTS "Users can view their own credit analysis" ON credit_analysis;
    DROP POLICY IF EXISTS "Users can view their own file uploads" ON file_uploads;
    
    -- Create policies
    CREATE POLICY "Users can view their own profile" ON user_profiles FOR ALL USING (user_id = current_setting('app.user_id')::UUID);
    CREATE POLICY "Users can view their own income" ON income_entries FOR ALL USING (user_id = current_setting('app.user_id')::UUID);
    CREATE POLICY "Users can view their own expenses" ON expense_entries FOR ALL USING (user_id = current_setting('app.user_id')::UUID);
    CREATE POLICY "Users can view their own budgets" ON budget_allocations FOR ALL USING (user_id = current_setting('app.user_id')::UUID);
    CREATE POLICY "Users can view their own goals" ON user_goals FOR ALL USING (user_id = current_setting('app.user_id')::UUID);
    CREATE POLICY "Users can view their own credit scores" ON credit_scores FOR ALL USING (user_id = current_setting('app.user_id')::UUID);
    CREATE POLICY "Users can view their own chat sessions" ON chat_sessions FOR ALL USING (user_id = current_setting('app.user_id')::UUID);
    CREATE POLICY "Users can view their own chat messages" ON chat_messages FOR ALL USING (user_id = current_setting('app.user_id')::UUID);
    CREATE POLICY "Users can view their own credit analysis" ON credit_analysis FOR ALL USING (user_id = current_setting('app.user_id')::UUID);
    CREATE POLICY "Users can view their own file uploads" ON file_uploads FOR ALL USING (user_id = current_setting('app.user_id')::UUID);
END $$;

-- Success message
SELECT 'TaxSage database schema created successfully! 🎉' as result;
SELECT 'Tables created: ' || count(*) || ' tables' as tables_count FROM information_schema.tables WHERE table_schema = 'public' AND table_name LIKE '%';
